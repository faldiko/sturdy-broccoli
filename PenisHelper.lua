...
local FishHelperCheckBox = {
    { u8'test', u8'test', render }
}

local CentralMarketCheckBox = {
    { u8'test', u8'test', render }
}

local RouletteCheckBox = {
    { u8'test', u8'test', render }
}

local OthersCheckBox = {
    { u8'Рыбий Глаз', u8'Добавлю', camera_zoom_enabled }
}
...
imgui.OnFrame(function() return Window[0] end, function()
    ...
    if imgui.BeginTabBar('Tabs') then
        if imgui.BeginTabItem(u8'Хелпер лесоруб') then
            ...
            for i, checkbox in ipairs(LesorubCheckbox) do
                if i ~= 3 then
        ...
        if imgui.BeginTabItem(u8'Хелпер рыболов') then
		            for i, checkbox in ipairs(FishHelperCheckBox) do
                if i ~= 3 then
                    imgui.Checkbox(checkbox[1], checkbox[3])
                    imgui.SameLine()
                    imgui.Text('(?)')
        ...
        if imgui.BeginTabItem(u8'ЦР') then
		            for i, checkbox in ipairs(CentralMarketCheckBox) do
                if i ~= 3 then
                    imgui.Checkbox(checkbox[1], checkbox[3])
                    imgui.SameLine()
                    imgui.Text('(?)')
        ...
        if imgui.BeginTabItem(u8'Рулетки') then
		            for i, checkbox in ipairs(RouletteCheckBox) do
                if i ~= 3 then
                    imgui.Checkbox(checkbox[1], checkbox[3])
                    imgui.SameLine()
                    imgui.Text('(?)')
...
function safeThread(func, delay)
    lua_thread.create(function()
        ...
        while true do
            if isGameWindowForeground() then
                ...
                repeat wait(500) until isGameWindowForeground()
            end
            wait(delay or 0)
        end
    end)
...
function main()
    ...
    safeThread(RenderRadius, 10)
    safeThread(Collision, 100)
    safeThread(AutoPressH, 10)
	safeThread(CameraZoom, 10)
    safeThread(posadkaq, 0)
    safeThread(spilivanieq, 0)
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text) if prokachka[0] then if text:match('{FFFFFF}Будет готов через:') then sampSendDialogResponse(dialogId, 1, 1, nil) return false end end end
